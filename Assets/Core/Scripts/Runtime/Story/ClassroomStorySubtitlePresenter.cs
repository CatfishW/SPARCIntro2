...
namespace Blocks.Gameplay.Core.Story
{
    ...
    public sealed class ClassroomStorySubtitlePresenter : MonoBehaviour
    {
        private const float FadeDurationSeconds = 0.16f;
        private const float DefaultCharactersPerSecond = 54f;
        ...
        private float typewriterCharactersPerSecond = DefaultCharactersPerSecond;
        ...
        {
            EnsureUi();
            var displayName = string.IsNullOrWhiteSpace(request?.SpeakerDisplayName)
                ? request?.SpeakerId ?? string.Empty
            ...
            typewriterCharactersPerSecond = DefaultCharactersPerSecond;
            var closeDelay = request != null && request.AutoAdvance
            ...
            if (clip != null)
            {
                ...
                var clipDuration = Mathf.Max(0.25f, clip.length);
                closeDelay = request.AutoAdvance ? Mathf.Max(closeDelay, clipDuration + 0.05f) : clipDuration;

                if (!string.IsNullOrWhiteSpace(typewriter.FullText))
                {
                    typewriterCharactersPerSecond = Mathf.Max(
                        15f,
                        typewriter.FullText.Length / Mathf.Max(0.3f, clipDuration * 0.93f));
                }
            }
            else
            {
            ...
            autoCloseRemainingSeconds = closeDelay;
        }
