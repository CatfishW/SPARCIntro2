...
namespace Blocks.Gameplay.Core
{
    ...
    public class CoreAnimator : NetworkAnimator
    {
        ...
        {
            // Ensure the base NetworkAnimator has its reference before any network logic starts
            if (Animator == null)
            {
            ...
            {
                coreMovement = GetComponentInParent<CoreMovement>();
                if (coreMovement == null) Debug.LogError("[Core Animator] needs a CoreMovement component.");
            }

            if (soundDefFootstep == null)
            {
                Debug.LogWarning("[Core Animator] Footstep SoundDef is not assigned.");
            }
        }
        ...
        public void RebindAnimator(Animator animator, Avatar avatar = null, RuntimeAnimatorController runtimeAnimatorController = null)
        {
            if (animator == null)
            {
                Debug.LogError("[Core Animator] Cannot rebind to a null Animator.", this);
                return;
            }

            RuntimeAnimatorController controllerToApply = runtimeAnimatorController;
            if (controllerToApply == null && Animator != null)
            {
                controllerToApply = Animator.runtimeAnimatorController;
            }
